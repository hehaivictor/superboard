...
1. 投资委员会认为长期价值取决于数据和工作流嵌入，不在通用回复生成。
2. 产品委员会认为信任和可审计性是最大产品门槛。
3. 创业委员会认为应先拿 3 家设计伙伴付费试点，再谈渠道规模化。
...
|---|---|---|---|---|
| 商业与长期价值委员会 | 杰夫·贝索斯 | 材料涉及客户长期不变需求和服务质量。 | 客户响应速度、准确率、可控性 | 若客户已有成熟客服系统且切换成本过高，则商业判断降级。 |
| 战略与竞争委员会 | 迈克尔·波特 | 材料涉及替代力量、进入策略和差异化。 | 目标细分市场、替代方案、竞争结构 | 若差异只来自通用模型能力，则战略防御不足。 |
| 投资与风险委员会 | 查理·芒格 | 材料涉及坐席计费、激励和客户行为扭曲。 | 计费方式、使用激励、客户预算 | 若计费方式诱导客户减少使用，则定价假设降级。 |
...
- 代表观点：先从客户长期不变需求倒推，而不是从模型能力出发。
- 证据要求：客户响应速度、准确率、可控性和长期服务成本。
- 反证提醒：若客户已有成熟客服系统且不愿切换，进入策略必须调整。

### 迈克尔·波特

- 代表观点：通用 AI 回复不是战略定位，必须明确细分市场、替代力量和防御点。
- 证据要求：目标客户、替代方案、差异化来源和竞争反应。
- 反证提醒：若差异只来自通用模型能力，则不应按高防御业务判断。

### 查理·芒格

- 代表观点：坐席订阅可能与客户价值错配，必须检查激励后果。
- 证据要求：计费方式、客户预算、使用行为和销售激励。
- 反证提醒：若客户为了省钱减少坐席或减少使用，定价假设降级。

## 董事会审议信号图

```mermaid
flowchart TD
  A["最终建议：Pivot"] --> B["商业委员会：客户信任倒推"]
  A --> C["创业委员会：3 家设计伙伴"]
  A --> D["投资委员会：护城河不足"]
  A --> E["咨询委员会：细分市场优先"]
  A --> F["产品委员会：可审计建议回复"]
...
- 创业委员会更愿意快速试点，投资委员会要求先限制错误下行。
- 商业委员会强调客户信任体验，咨询委员会强调市场进入选择。
- 产品委员会倾向先做质检和建议回复，商业计划原设想更偏坐席 Copilot。
